...
void PopUp::init()
{
	...
	stream.open(CONFIG_FOLDER + POP_UP_FOLDER + "popUp.txt");
	...
	m_easyBtn->init(easyBtn, POP_UP_FOLDER);
	m_hardBtn->init(hardBtn, POP_UP_FOLDER);

	m_difficulty.texture = loadTexture(POP_UP_FOLDER + difficultyImg);
}
